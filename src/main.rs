...
fn main() {
  ...
  let t1: Transaction = Transaction::new(
      &keys_miner_reward, keys_some_address.public_key.clone(), 2);
  let t2: Transaction = Transaction::new(
      &keys_some_address, keys_another_address.public_key.clone(), 1);
  ...
  let t3: Transaction = Transaction::new(
      &keys_miner_reward, keys_another_address.public_key.clone(), 3);
  let t4: Transaction = Transaction::new(
      &keys_another_address, keys_some_address.public_key.clone(), 1);
  ...
  println!("Miner reward wallet ammount {}", blockchain.get_balance(&keys_miner_reward.public_key[..]).unwrap());
  println!("Some wallet ammount {}", blockchain.get_balance(&keys_some_address.public_key[..]).unwrap());
  println!("Another wallet ammount {}", blockchain.get_balance(&keys_another_address.public_key[..]).unwrap());
